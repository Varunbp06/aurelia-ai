...
async def test_clear_all_urls_response_schema(client, default_agent_id):
    ...
    assert "deleted_url_count" in data, (
        f"Response missing 'deleted_url_count' field. Got: {list(data.keys())}"
    )
    assert isinstance(data["message"], str), "message should be a string"
    assert isinstance(data["deleted_url_count"], int), "deleted_url_count should be an int"
    assert data["deleted_url_count"] == 2, (
        f"Expected deleted_url_count=2, got {data['deleted_url_count']}"
    )
...
def _import_scrapling_module():
    ...
    scrapling_path = os.path.join(os.path.dirname(__file__), "../../scrapling-service/main.py")
