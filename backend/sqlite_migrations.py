    ...
    try:
        ...
        if _table_exists(cursor, "admin_users") and _table_exists(cursor, "workspaces"):
            ...
            if row:
                ...
                cursor.execute(
                    "INSERT INTO workspaces (name, owner_email, created_at) VALUES ('Default Workspace', 'admin@aurelia.local', CURRENT_TIMESTAMP)"
                )
...
def _migrate_agents(cursor: sqlite3.Cursor):
    ...
    columns: list[tuple[str, str]] = [
        ...
        (
            "welcome_message",
            "TEXT DEFAULT '您好！我是Aurelia助手，有什么可以帮您的吗？'",
        ),
...
def _backfill_agents(cursor: sqlite3.Cursor):
    ...
    if "welcome_message" in col_names:
        cursor.execute(
            "UPDATE agents SET welcome_message = '您好！我是Aurelia助手，有什么可以帮您的吗？' "
