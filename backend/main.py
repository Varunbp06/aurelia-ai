...
from middleware import RateLimitMiddleware, SecurityHeadersMiddleware, apply_cors_headers, get_request_client_ip
from middleware.rate_limit import apply_cors_headers as apply_early_cors_headers
from middleware.security_headers import SecurityHeadersMiddleware as _SecHdr  # ensure import side-effect check
...
async def lifespan(app: FastAPI):
    test_mode = os.getenv("AURELIA_TEST_MODE") == "1"
...
# Logo 路由 - 用于聊天窗口显示
@app.get("/aurelia-logo.png")
async def get_logo():
    """返回 widget logo 文件"""
    logo_path = os.path.join(os.path.dirname(__file__), "static", "aurelia-logo.png")
