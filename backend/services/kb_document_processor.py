...
def get_embedding_api_key(agent: Agent | None) -> str | None:
    ...
    if embedding_provider == "jina":
        ...
        # Custom provider key is stored in siliconflow_api_key (via kb_setup)
        key = decrypt_api_key(getattr(agent, "siliconflow_api_key", None))
        if key:
            return key
        return decrypt_api_key(getattr(agent, "jina_api_key", None))
    return None
...
class KbDocumentProcessor:
    ...
    def save_uploaded_file(self, doc: KbDocument, content: bytes, ext: str) -> str:
        """Save bytes to disk, return storage_path. Path-traversal hardened."""
        ...
        # Strip directory components and sanitize to prevent traversal
        base = Path(filename).name
        safe_name = "".join(c for c in base if c.isalnum() or c in "._-").strip("._")[:200]
        if not safe_name:
            safe_name = f"upload.{ext}"
        elif not safe_name.lower().endswith(f".{ext.lower()}"):
            # Preserve single extension boundary (prevent double-extension masquerade)
            safe_name = f"{safe_name}.{ext}"
        path = (d / safe_name).resolve()
        # Enforce containment within UPLOAD_ROOT
        try:
