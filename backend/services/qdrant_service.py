...
def get_embedding_dimension(model: str) -> int:
    ...
    if "bge-m3" in m or "bge_m3" in m or "baai/bge-m3" in m:
        return 1024
    if "nemotron-3-embed" in m or "nemotron" in m:
        return 2048
    if "arctic-embed" in m:
        return 1024
...
class QdrantKbService:
    ...
    async def ensure_collection(self, kb_id: str, embedding_model: str) -> str:
        """幂等创建 collection。已存在则返回名称；维度不匹配则重建。"""
        ...
        try:
            ...
            if info:
                ...
                if existing_dim is not None and existing_dim != dim:
                    logger.warning(
                        f"Qdrant collection '{collection_name}' dimension mismatch (existing={existing_dim}, required={dim}) — recreating"
                    )
                    try:
