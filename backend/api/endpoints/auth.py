...
def _check_register_rate_limit(
    ...
    """In-memory sliding-window rate limit for registrations (Redis fallback)."""
    now = time.time()
    history = _login_attempt_history[f"register:{ip}"]
    while history and now - history[0] >= window_seconds:
        history.popleft()
    if not history:
        _login_attempt_history.pop(f"register:{ip}", None)
        return True, 0
    if len(history) >= max_attempts:
        retry_after = int(window_seconds - (now - history[0])) + 1
        return False, max(retry_after, 1)
...
async def register(
    ...
    # Rate-limit signups per client IP (Redis-first, in-memory fallback).
    ...
    reg_max = cfg.register_rate_limit_max_attempts
    reg_window = cfg.register_rate_limit_window_seconds
    try:
        if redis_svc is not None:
            allowed, _remaining = await redis_svc.check_rate_limit(
                f"register:ip:{ip}", max_requests=reg_max, window_seconds=reg_window
            )
            if not allowed:
                raise HTTPException(
                    status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                    detail=_("Too many attempts. Please try again later.", locale=locale),
                    headers={"Retry-After": str(reg_window)},
                )
        else:
        ...
        allowed, retry_after = _check_register_rate_limit(ip, reg_max, reg_window)
        if not allowed:
            raise HTTPException(
                status_code=status.HTTP_429_TOO_MANY_REQUESTS,
                detail=_("Too many attempts. Please try again later.", locale=locale),
    ...
    # - Always allowed if public_registration_enabled is True
    # - If public_registration_enabled is False, only allowed when bootstrap is required (no admins yet)
    from sqlalchemy import select, func

    admin_count_result = await db.execute(select(func.count(AdminUser.id)))
    admin_count = admin_count_result.scalar() or 0

    if not cfg.public_registration_enabled and admin_count > 0:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=_("Registration is disabled. Contact an administrator.", locale=locale),
        )
    ...
    async with _bootstrap_lock:
        ...
        # Every signup gets its own isolated workspace + quota (tenant isolation).
        workspace = Workspace(
            name=f"Workspace of {req.name or req.email}",
...
async def get_registration_settings(db: AsyncSession = Depends(get_db)):
    ...
    return RegistrationSettingsResponse(
        public_registration_enabled=bool(cfg.public_registration_enabled),
