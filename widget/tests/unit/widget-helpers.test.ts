/**
 * Unit tests for AureliaWidget helper functions.
 *
...
describe('Widget API Base Detection (conceptual tests)', () => {
  ...
  it('uses configured apiBase when provided', () => {
    ...
    const url = new URL('/aurelia-logo.png', `${apiBase}/`);
    expect(url.toString()).toBe('https://api.example.com/aurelia-logo.png');
  });
  ...
  it('builds logo URL from relative path', () => {
    ...
    const url = new URL('/aurelia-logo.png', origin);
    expect(url.toString()).toContain('/aurelia-logo.png');
  });
});
...
describe('Widget Storage Key Conventions', () => {
  it('generates session key per agent ID', () => {
    ...
    const storageKey = `aurelia_session_${agentId}`;
    expect(storageKey).toBe('aurelia_session_agt_0123456789ab');
  });

  it('visitor ID uses global key', () => {
    expect('aurelia_visitor_id').toBe('aurelia_visitor_id');
  });
});
