...
class AureliaWidget {
  ...
  private readonly VISITOR_STORAGE_KEY = 'aurelia_visitor_id';
  ...
  private createChatWindow() {
    ...
    const closeBtn = this.chatWindow.querySelector('.aurelia-close') as HTMLElement;
    this._closeBtnClickListener = () => this.close();
    closeBtn.addEventListener('click', this._closeBtnClickListener);

    const input = this.chatWindow.querySelector('.aurelia-input') as HTMLInputElement;
    const sendBtn = this.chatWindow.querySelector('.aurelia-send') as HTMLButtonElement;
  ...
  private open() {
    ...
    this.stopTitleBlink();
    this.updateUnreadBadge();
    const input = this.chatWindow?.querySelector('.aurelia-input') as HTMLInputElement | null;
    setTimeout(() => {
      input?.focus();
    }, 300);
  }
  ...
  private showThinkingIndicator(elapsed = 0): void {
    ...
    const messagesContainer = this.chatWindow?.querySelector('.aurelia-messages') as HTMLElement | null;
  ...
  private finalizeStreamingMessage(sources: Source[] = []): void {
    ...
    const messagesContainer = this.chatWindow?.querySelector('.aurelia-messages') as HTMLElement;
    messagesContainer.scrollTop = messagesContainer.scrollHeight;
  ...
  destroy() {
    ...
    const closeBtn = this.chatWindow?.querySelector('.aurelia-close') as HTMLElement | null;
    if (closeBtn && this._closeBtnClickListener) {
      closeBtn.removeEventListener('click', this._closeBtnClickListener);
    }
    const sendBtn = this.chatWindow?.querySelector('.aurelia-send') as HTMLButtonElement | null;
...
(window as any).AureliaWidget = AureliaWidget;
...
(function bootstrapAureliaWidget() {
  const globalWindow = window as Window & typeof globalThis & {
    __aureliaWidgetAutoInitScheduled?: boolean
  }
