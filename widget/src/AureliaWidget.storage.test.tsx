/**
 * Storage fallback tests for AureliaWidget
...
describe('AureliaWidget storage fallback', () => {
  ...
  afterEach(() => {
    ...
    const containers = document.querySelectorAll('#aurelia-widget-container');
    containers.forEach((el) => el.remove());
    const styles = document.querySelectorAll('#aurelia-widget-styles');
  ...
  it('does not throw during widget construction when localStorage.getItem throws', () => {
    ...
    const AureliaWidget = (window as any).AureliaWidget;

    expect(() => {
      new AureliaWidget({ agentId: 'test-agent' });
    }).not.toThrow();
  });
  ...
  it('creates container and button when storage access is denied', async () => {
    ...
    const AureliaWidget = (window as any).AureliaWidget;
    const widget = new AureliaWidget({ agentId: 'test-agent', apiBase: 'http://localhost:8000' });

    await widget.init();

    // Verify widget container exists
    const container = document.getElementById('aurelia-widget-container');
    expect(container).toBeTruthy();

    // Verify widget button exists
    const button = document.getElementById('aurelia-widget-button');
  ...
  it('falls back to memory storage and returns values during same page lifecycle', async () => {
    ...
    expect(store.has('aurelia_visitor_id')).toBe(true);
    const visitorId = store.get('aurelia_visitor_id');
    ...
    const widget2 = new AureliaWidget({ agentId: 'test-agent', apiBase: 'http://localhost:8000' });
    await widget2.init();

    // Both widgets should have the same visitor ID
    const visitorId2 = store.get('aurelia_visitor_id');
  ...
  it('maintains session_id across widget operations when storage works', async () => {
    ...
    store.set('aurelia_session_test-agent', 'session_12345');

    const widget = new AureliaWidget({ agentId: 'test-agent', apiBase: 'http://localhost:8000' });
    await widget.init();

    // Session should be loaded from storage
    expect(store.get('aurelia_session_test-agent')).toBe('session_12345');
  });
});
