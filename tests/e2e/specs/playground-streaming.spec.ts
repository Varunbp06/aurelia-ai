...
test.describe("Playground KB Context Retrieval", () => {
	test("chat request succeeds after KB setup with indexed content", async ({ page, request }) => {
		...
		// Requires a REAL Jina key: embeddings/retrieval hit the live Jina API.
		const jinaApiKey = process.env.E2E_JINA_API_KEY;
		test.skip(!jinaApiKey, "E2E_JINA_API_KEY not set - embedding-backed retrieval needs a real Jina API key");
