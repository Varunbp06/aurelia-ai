...
test.describe("URL Bug Fixes", () => {
	...
	test("Bug 2: Clear All uses POST method and returns deleted_count", async ({
		...
		// 7. Find and click "Clear All" button ("清空列表" zh-CN / "Clear All" en-US)
		const clearAllButton = page
			.getByRole("button", { name: /清空列表|Clear All|Clear List/i })
			.or(page.locator("button").filter({ hasText: /清空列表|Clear All|Clear List/i }));
		...
		// Verify response has message and a deleted count
		// (backend returns granular counters: deleted_url_count / deleted_files_count)
		const clearData = (await clearRes.json()) as {
			message: string;
			deleted_count?: number;
			deleted_url_count?: number;
		};
		expect(clearData.message).toBeTruthy();
		const deletedCount = clearData.deleted_count ?? clearData.deleted_url_count ?? 0;
