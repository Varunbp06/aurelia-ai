...
test.describe('Recent commit regressions', () => {
  ...
  test('auth language switcher works before login', async ({ page }) => {
    ...
    // The UI is intentionally English-only since the Aurelia redesign:
    // any stored zh-CN locale must migrate to en-US on boot (i18n/config.ts).
    await page.evaluate(() => localStorage.setItem('basjoo_locale', 'zh-CN'));
    await page.reload();
    await expect(page.getByRole('button', { name: /login|登录/i })).toBeVisible();
    const migratedLocale = await page.evaluate(() => localStorage.getItem('basjoo_locale'));
    expect(migratedLocale).toBe('en-US');
    const bodyAfterReload = await page.locator('body').innerText();
    expect(bodyAfterReload).not.toContain('登录');
