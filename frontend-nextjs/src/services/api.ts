...
class APIService {
	...
	private selectedAgentStorageKey = "aurelia_selected_agent_id";
	...
	private getStreamBaseUrl(): string {
		// Same-origin by default: Next.js rewrites proxy /api/* to BACKEND_PROXY_TARGET
		// (dev Docker and Vercel alike). Non-empty NEXT_PUBLIC_API_BASE_URL overrides.
		return this.baseUrl;
	}
	...
	setSelectedAgentId(agentId: string) {
		...
		window.dispatchEvent(
			new CustomEvent("aurelia-agent-changed", { detail: { agentId } }),
		);
	}
	...
	clearSelectedAgentId() {
		...
		window.dispatchEvent(
			new CustomEvent("aurelia-agent-changed", { detail: { agentId: null } }),
		);
	}
	...
	async clearAllUrls(
		...
		return this.request(`/api/v1/urls:clear_all?agent_id=${agentId}`, {
			...
			// Backend reports deleted_url_count / deleted_files_count; normalize
			// to the total removed for display purposes.
			const count =
				typeof data.deleted_count === "number"
					? data.deleted_count
					: (Number(data.deleted_url_count) || 0) +
						(Number(data.deleted_files_count) || 0);
			return { message: String(data.message ?? ""), deleted_count: count };
		});
	}
